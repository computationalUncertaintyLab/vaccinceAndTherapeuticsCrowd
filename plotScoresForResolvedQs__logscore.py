...
def plotData(individualRanks,consRanks,ax,W,legend,txtlabel):
    ...
    sns.stripplot(x="qid",y="logscore"
                  ,hue="expert"
                  ,dodge=True
                  ,data=individualRanks
                  ,palette = {0:"red",1:"blue"},alpha=0.5,ax=ax)

    for _ in ax.collections:
        _.set_label(s=None)   
    
    sns.boxplot(x="qid",y="logscore"
                  ,hue="expert"
                  ,dodge=True
                  ,data=individualRanks
                  ,linewidth=1
    ...
    for expert in [0,1,2]:
        ...
        if expert == 0:
            ...
            offset = 0
        ax.scatter( subset["order"]+offset, subset["logscore"]
                    ,s=35, marker=markers[expert], color = palette[expert], edgecolors="black", alpha=0.80,label=labels[expert] )


    ax.tick_params(which="both",labelsize=8)
    ax.set_xlabel("")

    ax.set_ylabel("Log score",fontsize=10)

    ax.set_yticks([-0.5,1,3,5])
    ax.set_ylim(-1,5.5)
...
if __name__ == "__main__":
    ...
    axs[1,0].set_yticks([-0.5,1,3,5,7,9])
    ...
    plt.savefig("./scores__log.pdf")
    plt.savefig("./scores__log.png",dpi=350)
    plt.savefig("./scores__log.tiff",dpi=350)
